...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $link = htmlspecialchars($_POST['bitly'] ?? '');
    $website = htmlspecialchars($_POST['website'] ?? '');
    $salesnumber = htmlspecialchars($_POST['salenumber'] ?? '');
    $salesemail = htmlspecialchars($_POST['sales_email'] ?? '');
    $salesname = htmlspecialchars($_POST['sales_name'] ?? '');

    $secondaryName = "";
    $secondaryEmail = "";

    if(!empty($_POST['secondarysales_name'] ))
    { 
        $secondaryName = htmlspecialchars($_POST['secondarysales_name'] ?? '');
        $secondaryEmail = htmlspecialchars($_POST['secondarysales_email'] ?? '');
    }
    ...
    $recipients = [
        ...
        ["name" => $salesname, "email" => $salesemail],
    ];
    
    // Check if additional email is not empty before adding
    if ($secondaryEmail != "") {
        $recipients[] = ["name" => $secondaryName, "email" => $secondaryEmail];
    }
